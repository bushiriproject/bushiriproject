/**
 * PROJECT BUSHIRI v3.2
 * MPESA/MIXX Captive Portal + NAT Router + VPS Verify
 * Bei: TZS 800 = Masaa 15
 * 
 * APSTA Mode: ESP32 inapokea internet kutoka modem (STA)
 *             na kusambaza kwa wateja (AP) kupitia NAT
 */
...
// NAT - lazima iwe kabla ya lwip headers nyingine
#include "lwip/lwip_napt.h"
#include "lwip/ip_addr.h"

// ==================== EDIT HAPA TU ====================
const char* AP_SSID      = "Bushiri WiFi";
const char* AP_PASS      = "";           // Weka "" kwa open network
...
const char* STA_SSID_ALT = "infinitynetwork";  // WiFi backup
const char* STA_PASS_ALT = ".kibushi1";
String ownerIP = "192.168.4.2";         // IP ya owner (free access)
// ======================================================

#define PROJECT_NAME  "BUSHIRI"
#define VERSION       "3.2.0"
#define MAX_CLIENTS   20
#define AP_IP_ADDR    0xC0A80401  // 192.168.4.1 in hex (network order after htonl)
...
bool addSession(String ip, unsigned long durationMs) {
  // Update existing session kama ipo
  for (int i = 0; i < sessionCount; i++) {
    if (sessions[i].ip == ip) {
...
// ==================== NAT ENABLE ====================
void enableNAT() {
  // Subiri kidogo ili AP ianzike kabisa
  delay(500);
  
  // Wezesha IP forwarding na NAT kwa AP interface (192.168.4.1)
  err_t ret = ip_napt_enable(htonl(AP_IP_ADDR), 1);
  if (ret == ERR_OK) {
    natEnabled = true;
    Serial.println("[NAT] Imewashwa! Wateja wanaweza kupata internet.");
  } else {
    natEnabled = false;
    Serial.println("[NAT] Imeshindwa kuwasha! Retry...");
    // Jaribu tena baada ya sekunde 2
    delay(2000);
    ret = ip_napt_enable(htonl(AP_IP_ADDR), 1);
    if (ret == ERR_OK) {
      natEnabled = true;
      Serial.println("[NAT] Imewashwa (retry)!");
    }
  }
}
...
void connectToInternet() {
  ...
  // Jaribu WiFi iliyohifadhiwa kwanza
  ...
  // Jaribu WiFi ya backup (hardcoded)
  ...
  if (WiFi.status() == WL_CONNECTED) {
    Serial.println("\n[WiFi] Backup connected: " + WiFi.localIP().toString());
  } else {
    Serial.println("\n[WiFi] Hakuna internet - portal itafanya kazi bila NAT");
  }
}

void maintainWiFi() {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("[WiFi] Imekatika - inajaribu kuunganika...");
    connectToInternet();
    // Re-enable NAT baada ya kuunganika tena
    if (WiFi.status() == WL_CONNECTED) {
      enableNAT();
    }
  }
...
void setup() {
  ...
  // Fungua preferences (NVS)
  prefs.begin("bushiri");

  // ----- HATUA 1: Washa WiFi katika mode APSTA -----
  WiFi.mode(WIFI_AP_STA);
  Serial.println("[WiFi] Mode: APSTA");

  // ----- HATUA 2: Sanidi Access Point (AP) -----
  IPAddress apIP(192, 168, 4, 1);
  IPAddress gateway(192, 168, 4, 1);
  IPAddress subnet(255, 255, 255, 0);
  WiFi.softAPConfig(apIP, gateway, subnet);
  
  // Washa AP - channel 6, hidden=0, max_conn=8
  if (strlen(AP_PASS) >= 8) {
    WiFi.softAP(AP_SSID, AP_PASS, 6, 0, 8);
  } else {
    WiFi.softAP(AP_SSID, NULL, 6, 0, 8); // Open network kama pass fupi
  }
  Serial.println("[AP] Imewashwa: " + String(AP_SSID));
  Serial.println("[AP] IP: " + WiFi.softAPIP().toString());

  // ----- HATUA 3: Unganika na internet (STA) -----
  connectToInternet();

  // ----- HATUA 4: Washa NAT kama internet ipo -----
  if (WiFi.status() == WL_CONNECTED) {
    delay(1000); // Subiri network ianzike
    enableNAT();
  } else {
    Serial.println("[NAT] Haitawashwa - hakuna internet");
  }

  // ----- HATUA 5: DNS Server (captive portal) -----
  dnsServer.setErrorReplyCode(DNSReplyCode::NoError);
  dnsServer.start(53, "*", apIP);
  Serial.println("[DNS] Imeanza - captive portal aktif");

  // ----- HATUA 6: Web server na OTA -----
  setupWebServer();
  setupOTA();

  Serial.println("[READY] BUSHIRI iko tayari!");
...
void loop() {
  ...
  // Heartbeat kila sekunde 30
  if (millis() - lastHeartbeat > 30000) {
    lastHeartbeat = millis();
    clientCount   = WiFi.softAPgetStationNum();

    Serial.printf("[Status] Clients:%d Sessions:%d NAT:%s WiFi:%s\n",
      clientCount, sessionCount,
      natEnabled ? "ON" : "OFF",
      WiFi.status() == WL_CONNECTED ? WiFi.SSID().c_str() : "Hakuna"
    );

    // Hakikisha WiFi na NAT vinafanya kazi
    maintainWiFi();

    // Re-enable NAT kila wakati kwa uhakika
    if (WiFi.status() == WL_CONNECTED && !natEnabled) {
      enableNAT();
    }
    // Refresh NAT periodically
    if (WiFi.status() == WL_CONNECTED) {
      ip_napt_enable(htonl(AP_IP_ADDR), 1);
    }
  }
...
void setupWebServer() {
  ...
  // Captive portal detection - iPhone/iOS
  ...
  server.begin();
  Serial.println("[WebServer] Imeanza kwenye port 80");
}
...
void wifiConfigPage() {
  ...
  html += "<input type='password' name='pass' value='' placeholder='Password'>";
